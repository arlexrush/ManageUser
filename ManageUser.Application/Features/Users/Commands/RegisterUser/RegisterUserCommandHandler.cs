...
{
    public class RegisterUserCommandHandler : ICommandWResultHandler<RegisterUserCommand, RegisterUserCommandResponse>
    {
            ...
            RoleManager<ApplicationRole> roleManager)
        {
            ...
            _domainEventPublisher = domainEventPublisher ?? throw new ArgumentNullException(nameof(domainEventPublisher), "Domain Event Publisher cannot be null.");
            ...
            _signInManager = signInManager ?? throw new ArgumentNullException(nameof(signInManager), "Sign In Manager cannot be null.");
            _roleManager = roleManager ?? throw new ArgumentNullException(nameof(roleManager), "Role Manager cannot be null.");
        }
        
        public async Task<RegisterUserCommandResponse> HandleAsync(RegisterUserCommand command, CancellationToken cancellationToken)
        {
            ...
            try
            {
                // Este flujo es para crear un usuario individual, que estará asociado a un tenant por defecto
                var tenant = new Tenant<ApplicationUser>
                {
                    Name = "DefaultTenantUser", // Default tenant name
                    IsActive = true,
                    IsDeleted = false,
                    LogoUrl = command.ImageUserUrl, // Default logo URL
                    ImageTenantId = command.ImageUserId, // Default image ID
                    AddressTenant = "DefaultTenantUser Address", // Dirección del inquilino (tenant) al que pertenece el usuario
                    CIFTenant = command.CIF, // CIF del inquilino (tenant) al que pertenece el usuario
                    Description = "This is a Defaul tenant for Individual users", // Descripción del inquilino (tenant) al que pertenece el usuario
                ...
                var user = new ApplicationUser
                {
                    ...
                    Address = new Address() { Street = command.Street, Number = command.Number, City = command.City, Country = country, PostalCode = command.PostalCode, State = command.State, Cca2Address = command.Cca2Address },
                    CIF = command.CIF, // CIF del usuario
                };
            ...
            {
                // Log the exception
                _logger.LogError(ex, "An error occurred while handling the RegisterUserCommand.");
                throw ex;
            }
